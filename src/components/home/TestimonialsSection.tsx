...
const TestimonialsSection = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }).map((_, index) => (
      <svg 
        key={index} 
        className={`w-5 h-5 ${index < rating ? 'text-accent-500' : 'text-gray-300'}`} 
        fill="currentColor" 
        viewBox="0 0 20 20" 
    ...
    <section className="py-20 md:py-28 bg-gray-50">
      <div className="container mx-auto px-4 md:px-6">
        <SectionHeader
          ...
          subtitle="Don't just take our word for it. Hear from our satisfied clients about their experience working with SolidBuild Co."
        />
