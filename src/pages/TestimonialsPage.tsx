	...
	const renderStars = (rating: number) => {
		return Array.from({ length: 5 }).map((_, index) => (
			<Star
				...
				size={20}
				className={`${
					index < rating ? "text-accent-500 fill-accent-500" : "text-gray-300"
				}`}
			/>
		));
	};
	...
	return (
		<motion.div
			...
			<PageHeader
				...
				subtitle="Hear what our clients have to say about their experience working with GreenChack"
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12">
						{Array.from({ length: 8 }).map((_, index) => (
							<motion.div
								...
								<div className="text-primary-900 font-bold text-xl">
									Client Logo
								</div>
							</motion.div>
						))}
				...
				<div className="container mx-auto px-4 md:px-6 text-center">
					<motion.div
						...
						<p className="text-lg text-gray-600 mb-8">
							Contact us today to discuss your construction project and
							experience the SolidBuild difference.
						</p>
						<motion.div
